...
plt.xlabel(f"PC1 ({var_pct[0]:.1f}% variance)")
plt.ylabel(f"PC2 ({var_pct[1]:.1f}% variance)")
plt.title("PCA (DESeq2-style VST)")
plt.legend(title="Genotype")
plt.tight_layout()
plt.savefig("figures/pca.png", dpi=150)
plt.close()

print(f"PC1 explains {var_pct[0]:.1f}% of variance")
print(f"PC2 explains {var_pct[1]:.1f}% of variance")
print("Wrote figures/pca.png")